...
namespace gsstl
{
    ...
    string &string::operator += (const string &rhs) {
        if (_buffer) {
            if (rhs._buffer && rhs._size > 0) {
                ...
                memcpy(_buffer + _size, rhs._buffer, rhs._size + 1);
    ...
    string &string::operator += (const char *rhs) {
        if (_buffer) {
            if (rhs) {
                size_t rhssize = utf8size(rhs) - 1;
                
                if (rhssize > 0) {
                    size_t newSize = _size + rhssize;
                    
                    if (newSize >= _capacity) {
                        reserve(newSize + 1);
                    }
                    
                    memcpy(_buffer + _size, rhs, rhssize + 1);
