	...
	namespace Core
	{
		...
		class GS_API GS // this naming should be discussed, it was chosen because we need the static object with the name "GS"
		{
			public:
				...
				#if defined(GS_USE_STD_FUNCTION)
					typedef gsstl::function<void(GS&)> t_NotConnectedCallback;
				#else
					typedef void(*t_NotConnectedCallback)(GS&);
				#endif /* GS_USE_STD_FUNCTION */

				/// Callback which is triggered whenever the connection phase fails
				t_NotConnectedCallback GameSparksNotConnected;
