...
void GameSparksNotConnected_Static(GameSparks::Core::GS& gsInstance)
{
	UE_LOG(UGameSparksModuleLog, Warning, TEXT("%s"), TEXT("GameSparks::GameSparksNotConnected_Static"));
	UGameSparksModule::GetModulePtr()->SendGameSparksNotConnectedToComponents();
	UGameSparksModule::GetModulePtr()->TriggerOnGameSparksNotConnectedDelegates();
}
...
void UGameSparksModule::StartupModule()
{
	...
	GS.GameSparksNotConnected = GameSparksNotConnected_Static;
...
void UGameSparksModule::Initialize(FString apikey, FString secret, bool previewServer, bool clearCachedAuthentication, bool dontRetryAfterDisconnect)
{
    if(!isInitialised){
		...
		this->platform->SetDontRetryAfterDisconnect(dontRetryAfterDisconnect);
...
void UGameSparksModule::SendGameSparksNotConnectedToComponents()
{
	for (TObjectIterator<UGameSparksComponent> Itr; Itr; ++Itr)
	{
#if GS_UE_VERSION < GS_MAKE_VERSION(4, 14) // since 4.14 auto-generated enums no longer use TEnumAsByte
...
#endif
		{
			Itr->OnGameSparksNotConnectedDelegate.Broadcast();
		}
	}
