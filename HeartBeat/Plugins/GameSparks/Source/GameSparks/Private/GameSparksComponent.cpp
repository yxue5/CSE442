...
void UGameSparksComponent::Connect(FString apikey, FString secret, bool previewServer, bool clearCachedAuthentication, bool dontRetryAfterDisconnect)
{
    if (UGameSparksModule::GetModulePtr() != nullptr){
		UGameSparksModule::GetModulePtr()->Initialize(apikey, secret, previewServer, clearCachedAuthentication, dontRetryAfterDisconnect);
    }
}

void UGameSparksComponent::ConnectWithCredential(FString apikey, FString secret, FString credential, bool previewServer, bool clearCachedAuthentication, bool dontRetryAfterDisconnect)
{
    if (UGameSparksModule::GetModulePtr() != nullptr){
