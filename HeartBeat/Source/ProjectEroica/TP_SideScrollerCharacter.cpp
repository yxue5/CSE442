...
void ATP_SideScrollerCharacter::StartHit()
{
	//allow weapon to start detecting collisions
	CharWeapon->CapsuleComponent->SetCollisionResponseToChannel(ECollisionChannel::ECC_Pawn, ECR_Overlap);
	attackState = "Attacking";
}

void ATP_SideScrollerCharacter::EndHit()
{
	//make weapon stop detecting collisions
...
int ATP_SideScrollerCharacter::getLeftRight()
{
	float f = GetActorRotation().Yaw;
	if (f > 1) {
		return 1;
	}
	else return -1;
...
void ATP_SideScrollerCharacter::NotifyHit(UPrimitiveComponent * MyComp, AActor * Other, UPrimitiveComponent * OtherComp, bool bSelfMoved, FVector HitLocation, FVector HitNormal, FVector NormalImpulse, const FHitResult & Hit)
{
	...
	//else if we're landing on the floor
		...
		canRocket = true;
		GetCapsuleComponent()->SetCollisionResponseToChannel(ECollisionChannel::ECC_Pawn, ECollisionResponse::ECR_Block);
		if (State == Jumping || State == Knockup || State == JumpStab) {
			if (State == JumpStab) {
				GetWorld()->GetTimerManager().ClearTimer(AttackingHandle);
...
void ATP_SideScrollerCharacter::stopAttack() {
	...
	AttackHandle->attackLaunch = FVector(0,0,0);
...
void ATP_SideScrollerCharacter::Attack()
{
	...
	if (newState != State) {
		...
		//GetWorld()->GetTimerManager().ClearTimer(EndMovementHandle);
		if (AttackHandle->attackLaunch != FVector::ZeroVector) {
			LaunchCharacter(AttackHandle->attackLaunch, true, true);
		}
		State = newState;
		...
		//Sets end movement handle properly
...
void ATP_SideScrollerCharacter::MoveRight(float Value)
{
	if (GetWorld()->GetTimerManager().GetTimerRemaining(EndMovementHandle) <= 0.f && GetWorld()->GetTimerManager().GetTimerRemaining(AttackingHandle) <= 0.f && Value != 0) {
		...
		UE_LOG(LogTemp, Warning, TEXT("EndMovement: %f"), GetWorld()->GetTimerManager().GetTimerRemaining(EndMovementHandle));
		UE_LOG(LogTemp, Warning, TEXT("AttackingHandle: %f"), GetWorld()->GetTimerManager().GetTimerRemaining(AttackingHandle));
	}
}
